...
npm run cf:types         # 從 wrangler.jsonc 生成 Cloudflare binding 型別（改 wrangler.jsonc 後需手動跑一次）
...
### Binding 型別 — `npm run cf:types`

`src/server/types.ts` 的 `AppEnv` 和 DO 都使用 `Cloudflare.Env`，由 `wrangler types` 從 `wrangler.jsonc` 自動生成到 `worker-configuration.d.ts`。**不要手寫 Env interface。** 修改 `wrangler.jsonc` 的 bindings（新增/刪除/重命名 D1、R2、Queue、DO、vars）後，必須跑 `npm run cf:types` 重新生成型別。

### Cloudflare AI Gateway — 認證與 Provider 路由

所有 AI 呼叫都透過 Cloudflare AI Gateway（`aiClient.ts` 的 `getGatewayBaseUrl()`）。Gateway 支援兩種 provider：

| Provider             | URL Path                                                     | Model 格式                                | Auth Headers                                                           |
| -------------------- | ------------------------------------------------------------ | ----------------------------------------- | ---------------------------------------------------------------------- |
| **Google AI Studio** | `/google-ai-studio/...` 或 compat `/compat/chat/completions` | `google-ai-studio/gemini-2.5-flash`       | `cf-aig-authorization`                                                 |
| **OpenRouter**       | `/openrouter/v1/chat/completions`                            | `google/gemini-3.1-flash-lite-preview` 等 | `cf-aig-authorization` **+** `cf-aig-byok-alias: lex-draft-openrouter` |

**重要**：OpenRouter 的 stored key 必須帶 `cf-aig-byok-alias` header 指定 key alias，否則會得到 401 "No cookie auth credentials found"。Google AI Studio 不需要此 header。
...
| **cases**          | id  | user_id, title, case_number, court, plaintiff, defendant, client_role, case_instructions, case_summary, division, template_id | law_refs, timeline, undisputed_facts, information_gaps                                                 |
