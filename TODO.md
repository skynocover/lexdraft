...
- 點擊引用後 開啟該檔案在右側 並highlight那部分(確認好不好做 或是不需要) 如果固定兩個視窗 左邊是書狀 右邊是檔案預覽或是法條預覽
- 產生時間軸 獨立放置
...
UI
- 把左/右欄改成更淡的深灰層級，並降低邊框/陰影密度 讓畫面更接近大眾
- 右下角顯示字數統計 類似word
- 書狀大綱預覽 類似word pages或是其他文書
- 把上傳檔案改到案件卷宗的右上角 點擊+的icon
- 不要用顏色來分類卷宗 太過混亂
- PDF 縮放功能
- 拖曳移動檔案功能 可以把卷宗自由拖曳到不同的位置