...
1. 使用 write_full_brief 工具一次完成整份書狀撰寫
   - 根據下方「範本選擇指引」選擇正確的 template_id 並決定 title
   - 使用者模糊說「寫書狀」時，根據案件的 client_role 推斷最適合的範本，用一句話確認即可
...
export class AgentDO extends DurableObject<Env> {
  ...
  private async runAgentLoop(
    ...
    // 2b. Load case_instructions + template_id for system prompt injection
    ...
    const caseTemplateId = caseRows[0]?.template_id?.trim() || '';
