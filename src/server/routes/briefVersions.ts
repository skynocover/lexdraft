...
import { createVersionSchema } from '../schemas/briefVersions';
...
briefVersionsRouter.post('/briefs/:briefId/versions', async (c) => {
  const briefId = c.req.param('briefId');
  const body = parseBody(await c.req.json(), createVersionSchema);
  const db = getDB(c.env.DB);

  // 讀取當前 brief
  const briefRows = await db.select().from(briefs).where(eq(briefs.id, briefId));
  if (!briefRows.length) throw notFound('書狀');

  const brief = briefRows[0];
  if (!brief.content_structured) {
    throw badRequest('書狀尚無內容');
  }
...
briefVersionsRouter.get('/brief-versions/:id', async (c) => {
  ...
  const rows = await db.select().from(briefVersions).where(eq(briefVersions.id, id));

  if (!rows.length) throw notFound('版本');

  const version = rows[0];
  return c.json({
    ...version,
    content_structured: version.content_structured ? JSON.parse(version.content_structured) : null,
  });
});
