...
export const runBriefPipeline = async (
  ...
  try {
    ...
    // Filter out system-managed sections (e.g. 證據方法) before writer loop
    // NOTE: must mutate store.sections because getContextForSection(i) indexes into it
    // Filter out system-managed sections AND sections already rendered by Template Renderer (Track 1)
    // e.g. 壹、訴之聲明 — Step 2 sometimes produces these despite prompt instructions
    const renderedHeadings = new Set(templateSectionParagraphs.map((p) => p.section));
    store.sections = store.sections.filter(
      (s) => !isAutoGenerated(s.section) && !renderedHeadings.has(s.section),
    );
