...
function FileListDisplay({ content }: { content: string }) {
  ...
  let files: Array<{ filename: string; category: string | null; status: string | null }> = []
  try {
    const parsed = JSON.parse(content)
    if (Array.isArray(parsed)) files = parsed
  } catch {
    return <pre className="max-h-32 overflow-auto whitespace-pre-wrap text-t2">{content.slice(0, 500)}</pre>
  }
    ...
    <div className="space-y-1">
      {files.map((f, i) => (
        <div key={i} className="flex items-center gap-2">
          <span className={`shrink-0 rounded px-1 py-0.5 text-[10px] font-medium ${getCategoryColor(f.category)}`}>
...
function getCategoryColor(category: string | null): string {
  switch (category) {
    case 'ours': return 'bg-ac/20 text-ac'
    case 'theirs': return 'bg-or/20 text-or'
    case 'court': return 'bg-pu/20 text-pu'
    case 'evidence': return 'bg-cy/20 text-cy'
    default: return 'bg-bg-4 text-t3'
  }
}
