  ...
  return (
    <footer className="flex h-[26px] shrink-0 items-center justify-between border-t border-bd bg-bg-1 px-4">
      <div className="flex items-center gap-4">
        <span className="text-xs text-t3">Model: Gemini 2.5 Flash</span>
      ...
      <div className="flex items-center gap-2">
        {isStreaming ? (
          <span className="flex items-center gap-1 text-xs text-yl">
            <span className="inline-block h-1.5 w-1.5 animate-pulse rounded-full bg-yl" />
            Streaming
          </span>
        ) : (
          <span className="text-xs text-gr">Ready</span>
        )}
      </div>
    </footer>
  );
