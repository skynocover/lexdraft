  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between px-1">
        <span className="text-[10px] font-medium text-t3">{cleanText(category)}</span>
        <span className="text-[10px] text-t3">{formatAmount(groupTotal)}</span>
      </div>
      {items.map((d) => (
  ...
  return (
    <div className="rounded border border-bd bg-bg-2">
      ...
      >
        ...
        <span className="shrink-0 text-xs font-medium text-ac">
      ...
      {expanded && (
        <div className="space-y-1.5 border-t border-bd px-3 py-2">
          {damage.basis && (
            <div>
              <span className="text-[10px] font-medium text-t3">依據</span>
