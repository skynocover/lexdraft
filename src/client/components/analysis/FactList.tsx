...
export const FactList = ({ facts }: { facts: StructuredFact[] }) => {
  return (
    <div>
      <span className="text-xs font-medium text-t3">事實爭議</span>
      <div className="mt-1 space-y-1.5">
        {facts.map((fact) => {
          const style = ASSERTION_STYLES[fact.assertion_type] || ASSERTION_STYLES['主張'];
          return (
            <div key={fact.id} className="rounded border border-bd/50 bg-bg-1 px-2.5 py-2">
              <div className="flex items-start gap-1.5">
                <span
                  className={`mt-0.5 shrink-0 rounded px-1 py-0.5 text-xs font-medium ${style.bg} ${style.text}`}
                >
                  {fact.assertion_type}
                </span>
                <p className="flex-1 text-sm leading-relaxed text-t2">{fact.description}</p>
              </div>

              {fact.source_side !== '中立' && (
                <span className="mt-1 inline-block text-xs text-t3">來源：{fact.source_side}</span>
              )}
              ...
              {fact.disputed_by && (
                <div className="mt-1 flex items-start gap-1 rounded bg-amber-500/5 px-1.5 py-1">
                  ...
                  <p className="text-xs leading-relaxed text-or">{fact.disputed_by}</p>
                </div>
              )}
              ...
              {fact.evidence.length > 0 && (
                <div className="mt-1 flex flex-wrap items-center gap-1">
                  ...
                  {fact.evidence.map((e, i) => (
                    <span key={i} className="rounded bg-bg-3 px-1 py-0.5 text-xs text-t3">
                      {e}
                    </span>
                  ))}
                </div>
