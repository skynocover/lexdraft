...
export const FactList = ({ facts }: { facts: StructuredFact[] }) => {
  return (
    <div>
      <span className="text-[11px] font-medium text-t3">事實爭議</span>
          ...
          return (
            <div key={fact.id} className="rounded border border-bd/50 bg-bg-1 px-2 py-1.5">
              <div className="flex items-start gap-1.5">
                <span
                  className={`mt-0.5 shrink-0 rounded px-1 py-0.5 text-[11px] font-medium ${style.bg} ${style.text}`}
                >
                  {fact.assertion_type}
                </span>
              ...
              {fact.source_side !== '中立' && (
                <span className="mt-1 inline-block text-[11px] text-t3">
                  來源：{fact.source_side}
                </span>
              )}
              ...
              {fact.disputed_by && (
                <div className="mt-1 flex items-start gap-1 rounded bg-amber-500/5 px-1.5 py-1">
                  ...
                  <p className="text-[11px] leading-relaxed text-or">{fact.disputed_by}</p>
                </div>
              )}
              ...
              {fact.evidence.length > 0 && (
                <div className="mt-1 flex flex-wrap items-center gap-1">
                  ...
                  {fact.evidence.map((e, i) => (
                    <span key={i} className="rounded bg-bg-3 px-1 py-0.5 text-[11px] text-t3">
                      {e}
                    </span>
                  ))}
                </div>
